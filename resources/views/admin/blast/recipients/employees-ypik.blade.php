...
@section('title', __('app.blast.employee_ypik_title'))
...
<div class="ypk-page">
    <div class="ypk-head">
            <div>
                <div class="ypk-head-title">{{ __('app.blast.employee_ypik_title') }}</div>
                <div class="ypk-head-sub">{{ __('app.blast.employee_ypik_subtitle') }}</div>
            </div>
        <div class="ypk-head-actions">
            ...
            <a href="{{ route('admin.blast.recipients.employees-ypik-pamjaya.index') }}" class="ypk-btn ghost">
                <i class="fas fa-id-card"></i> {{ __('app.blast.data_ypik_pamjaya') }}
            </a>
        </div>
    </div>
                ...
                <div class="ypk-import-wrap">
                    ...
                    <form action="{{ route('admin.blast.recipients.employees-ypik.import') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <label class="ypk-btn ypk-file-btn" style="background:#fff;border-color:#99f6e4;color:#0f766e;">
                            <i class="fas fa-file-import"></i> {{ __('app.blast.import_ypik_employee_excel') }}
                            <input
                                type="file"
                                name="file"
                                accept=".xlsx,.xls,.csv"
                    ...
                    <form method="POST" action="{{ route('admin.blast.recipients.employees-ypik.destroy-all') }}" data-confirm-message="{{ __('app.blast.delete_all_ypik_confirm') }}" onsubmit="return confirm(this.dataset.confirmMessage)">
                        @csrf
                        @method('DELETE')
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex;gap:6px;">
                                        ...
                                        <form method="POST" action="{{ route('admin.blast.recipients.employees-ypik.destroy', $employee->id) }}" data-confirm-message="{{ __('app.blast.delete_employee_ypik_confirm') }}" onsubmit="return confirm(this.dataset.confirmMessage)">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="ypk-empty">
                                    {!! __('app.blast.no_employee_data_ypik', ['file' => '<b>karyawanypik aja.xlsx</b>']) !!}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
